...
# Add these new items one at a time.  Print the list after each step to 
# confirm the changes.
...
# Use list concatenation to add these three items to the 
# list all at once: [False, -4.6, '87'].

adding_practice = adding_practice + [False, -4.6, '87']
print(adding_practice)

# Use the cargo_hold list for the next set o f exercises.
cargo_hold = ['oxygen tanks', 'space suits', 'parrot', 'instruction manual', 'meal packs', 'slinky', 'security blanket']

# Use bracket notation to replace 'slinky' in the list with 'space tether'. 
# Print the list to confirm the change.

cargo_hold[5] = 'space tether'
print(cargo_hold)

# Remove the last item from the list with pop. 
# Print the element removed and the updated list.

removed_item = cargo_hold.pop()
print("Removed item:", removed_item)
print("Updated list:", cargo_hold)

# Remove the first item from the list with pop. 
# Print the element removed and the updated list.
...
# Add the items 1138 and ‘20 meters’ to the the list - the number at the start and 
...
# Use the remove method to take the parrot out of cargo_hold, 
...
# Use .format() to print the final list and its length. 
